...
public class MediaTranscoderEngine {
    ...
    public void transcodeVideo(String outputPath, MediaFormatStrategy formatStrategy) throws IOException, InterruptedException {
        ...
        try {
            ...
            setupMetadata(); // since we put our input into retriever and extractor at the same spot extractor got released
    ...
    private void runPipelines() throws InterruptedException {
        ...
        while (!((mVideoTrackTranscoder != null && mVideoTrackTranscoder.isFinished()) || (mAudioTrackTranscoder != null && mAudioTrackTranscoder.isFinished()))) {
            boolean stepped = (mVideoTrackTranscoder != null && mVideoTrackTranscoder.stepPipeline())
            ...
            if (mDurationUs > 0 && loopCount % PROGRESS_INTERVAL_STEPS == 0) {
                ...
                if (mVideoTrackTranscoder != null) {
                    videoProgress = mVideoTrackTranscoder.isFinished() ? 1.0 : Math.min(1.0, (double) mVideoTrackTranscoder.getWrittenPresentationTimeUs() / mDurationUs);
                }
